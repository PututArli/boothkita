...
export default function PhotoboothRoom({ roomId, roomCode, roomExpiresAt }: Props) {
  ...
  return (
    <>
      ...
      {/* Global Bottom-Left Utilities (Language & Fullscreen) */}
      <div style={{ position: 'fixed', bottom: 'max(24px, env(safe-area-inset-bottom))', left: 24, zIndex: 90, display: 'flex', gap: 8, alignItems: 'center' }}>
        <button 
          onClick={toggleFullscreen} 
          style={{ width: 40, height: 40, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)', borderRadius: '50%', background: isFullscreen ? 'var(--text)' : 'var(--surface)', color: isFullscreen ? 'var(--bg)' : 'var(--text)', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
        ...
        <div style={{ display: 'flex', gap: 4, background: 'var(--surface)', padding: 4, borderRadius: 100, border: '1px solid var(--border)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          <button onClick={() => setLang('id')} style={{ padding: '6px 12px', border: 'none', borderRadius: 100, background: lang === 'id' ? 'var(--text)' : 'transparent', color: lang === 'id' ? 'var(--bg)' : 'var(--text)', fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s' }}>ID</button>
          <button onClick={() => setLang('en')} style={{ padding: '6px 12px', border: 'none', borderRadius: 100, background: lang === 'en' ? 'var(--text)' : 'transparent', color: lang === 'en' ? 'var(--bg)' : 'var(--text)', fontWeight: 700, fontSize: 13, cursor: 'pointer', transition: 'all 0.2s' }}>EN</button>
        </div>
      </div>
    </>
  );
